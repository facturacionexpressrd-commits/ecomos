 ...
 * Activate (resume) a Meta campaign
 *
 * Body:
 * - storeId: string
...
export async function POST(req: NextRequest) {
  try {
    ...
    await metaClient.updateCampaign(metaCampaignId, accessToken, { status: "ACTIVE" });
    ...
    return NextResponse.json(
      { error: "Failed to activate campaign", details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
  }
}
